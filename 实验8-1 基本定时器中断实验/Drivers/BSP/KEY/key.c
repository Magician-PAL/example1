...
void key_init(void)
{
    ...
    HAL_GPIO_Init(WKUP_GPIO_PORT, &gpio_init_struct);           /* WKUP引脚模式设置,下拉输入 */
		...
		signal_filter1.active_level = 0;
		signal_filter1.sampling_time = 3;
		signal_filter1.load_time = 3;
		signal_filter1.last_state = 0;
}
...
uint8_t SignalFilter(uint8_t pin_val, Signal_Filter *pDetectinputinfo)
{
	  uint8_t portState = 0;
	
		if(pDetectinputinfo == NULL)
		{
				return 0;
		}
		
		portState = (pin_val == pDetectinputinfo->active_level);
		
		if ( portState == pDetectinputinfo->last_state)
		{
		   pDetectinputinfo->sampling_time = pDetectinputinfo->load_time;
		}
		else
		{
		   if(pDetectinputinfo->sampling_time > 0)
			 {
			    pDetectinputinfo->sampling_time--;
			 }
			 else
			{
				pDetectinputinfo->last_state = portState;
				pDetectinputinfo->sampling_time = pDetectinputinfo->load_time;
				return 1;
			}			
		}
